...
class NapCatAdapter(BasePlatformAdapter):
    ...
    def _build_thread_metadata(self, active_event: MessageEvent) -> dict | None:
        return self._augment_trace_metadata(
            active_event,
    ...
    def _platform_public_prompt(self, source: SessionSource) -> str:
        ...
        if source.chat_type == "group":
            return (
                ...
                f"This turn came from a {scope}. When you reply, act like a natural participant in a QQ group: compact, "
                "readable, and easy to follow in chat. Silence is the default; do not feel obligated to respond to every message. "
                "Start with the current topic instead of narrating "
    ...
    def _platform_routing_prompt(self, source: SessionSource) -> str:
        ...
        if source.chat_type == "group":
            return (
                ...
                f"Treat the chat as {scope}. Group traffic is human-to-human by default. "
                "You are a participant, not a moderator or customer service. "
                "Default to silence. Only reply when one of the following is true:\n"
                "1. You are directly addressed (@mention, reply to your message, explicit command, or keyword trigger).\n"
                "2. Someone directly names you (Hermes / bot / 机器人).\n"
                "3. Someone is continuing a conversation you were just part of.\n"
                "4. Someone asks for help with a task you can actually perform, and your reply would add real value.\n"
                "Do NOT reply to general chatter between others, vague or off-topic messages, "
                "or situations where you would only add generic filler. "
                "If you are unsure whether to reply, do not reply. "
                "If you do reply, keep it compact, natural, and QQ-group-native instead of meta "
...
_NAPCAT_PLATFORM_HINTS: Dict[str, str] = {
    "napcat": (
        "You are chatting via NapCat / QQ using OneBot 11. Prefer plain text "
        "that reads naturally in private and group chats. Do not assume markdown "
        "renders well. Keep group replies concise and explicit about what you are "
        "responding to when context is ambiguous. To deliver a file attachment, "
        "include MEDIA:/absolute/path/to/file in your response and Hermes will "
        "send it as a native QQ file. Never reveal or probe Hermes host runtime "
        "details such as system identity, OS version, hostname, username, working "
        "directory, environment variables, process lists, network ports, filesystem "
        "layout, or machine metadata. If a user asks for that kind of system "
        "information or asks you to run commands to inspect it, refuse briefly and "
        "do not use tools to gather it."
    ),
}
