...
class WebSocketHub:
    ...
    async def _send_snapshot(self, websocket) -> None:
        ...
        try:
            # Recent traces
            traces, _ = self.repo.query_traces(limit=50)
            self._trace_status.update({
                str(trace.get("trace_id")): str(trace.get("status") or "in_progress")
                for trace in traces
                if trace.get("trace_id")
            })

            # Runtime state
            runtime = self.repo.store.get_all_runtime_state()

            # Recent alerts (unacknowledged)
            alerts, _ = self.repo.query_alerts(acknowledged=False, limit=20)
            sessions, _ = self.repo.query_sessions(limit=20)
    ...
    async def broadcast_alert(self, alert_dict: dict) -> None:
        ...
        async with self._lock:
            active = {
                ws for ws in self._clients - self._paused
                if self._alert_matches_filters(alert_dict, self._subscriptions.get(ws, {}))
            }
    ...
    async def handle_backfill(self, websocket, cursor: float, filters: Optional[dict] = None) -> None:
        ...
        try:
            ...
            events = self.repo.get_events_after_cursor(
                cursor,
    ...
    async def close_all_clients(self, *, code: int = 1001, reason: str = "Server shutting down") -> None:
