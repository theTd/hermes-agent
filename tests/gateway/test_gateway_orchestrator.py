...
def test_orchestrator_prompt_hides_model_details():
    runner = _make_runner()
    system_prompt = _support(runner).build_system_prompt()
    ...
    assert '{"action":"spawn","payload":{"goal":"Generate an image for the user based on their prompt"},"complexity_score":12}' in system_prompt
    assert '{"action":"spawn","payload":{"goal":"Convert the provided text to speech and return the audio"},"complexity_score":11}' in system_prompt
    assert '{"action":"spawn","payload":{"goal":"Transcribe the uploaded audio clip to text"},"complexity_score":10}' in system_prompt
    assert "default to action=ignore unless the message is clearly related to the agent in context" in system_prompt
    assert "prefetched group memory explicitly defines that exact short message as a fixed trigger/keyword/command for Hermes" in system_prompt
...
def test_orchestrator_turn_system_context_uses_compact_child_board():
    runner = _make_runner()
    session = _extension(runner).get_or_create_orchestrator_session("agent:main:napcat:dm:chat-1")
    session_entry = _make_session_entry()
    ...
    assert '"tools_seen": ["terminal", "file_read"]' in message
    assert '"current_activity": "Inspecting gateway flow"' in message
...
def test_orchestrator_turn_system_context_excludes_platform_reply_style():
    ...
    assert "Decide how to handle this gateway turn." not in message
...
def test_orchestrator_turn_system_context_puts_structured_context_before_routing_prompt():
    ...
    structured_idx = message.index("[Context note: Current gateway turn context.")
    memory_idx = message.index("[Context note: Prefetched memory context")
    routing_idx = message.index("[Routing note: reuse existing child status when available.]")

    assert "[Platform note: NapCat platform conversation rules.]" not in message
    assert structured_idx < memory_idx < routing_idx


def test_orchestrator_turn_system_context_includes_running_full_agent_status():
    runner = _make_runner()
    session = _extension(runner).get_or_create_orchestrator_session("agent:main:napcat:dm:chat-1")
    session_entry = _make_session_entry()
...
def test_orchestrator_turn_system_context_includes_memory_context_block():
    runner = _make_runner()
    session = _extension(runner).get_or_create_orchestrator_session("agent:main:napcat:dm:chat-1")
    session_entry = _make_session_entry()
...
def test_orchestrator_memory_context_snapshot_includes_provider_debug_details(monkeypatch):
    ...
    assert captured["shutdown_all"] is True
...
async def test_run_orchestrator_agent_compacts_history_tool_noise(monkeypatch):
    ...
    # Consecutive assistant entries are merged so the payload to the LLM
    # alternates user/assistant roles (the prior structure had two
    # back-to-back assistant entries, which some providers reject or silently
    # concatenate).
    assert compact_history == [
        {"role": "user", "content": "please inspect the repo"},
        {
            "role": "assistant",
            "content": (
                '{"action":"reply","payload":{"text":"Tool activity summary: used terminal."},"complexity_score":1}'
                "\n"
                '{"action":"reply","payload":{"text":"I found the relevant files."},"complexity_score":1}'
            ),
        },
    ]
...
def test_compact_orchestrator_history_keeps_spawn_summary_as_canonical_decision():
    ...
    assert compact == [
        {
            "role": "assistant",
            "content": '{"action":"spawn","payload":{"goal":"analyze the repo deeply"},"complexity_score":72}',
        },
    ]
...
def test_compact_orchestrator_history_recovers_reply_from_internal_summary():
    ...
    assert compact == [
        {
            "role": "assistant",
            "content": '{"action":"reply","payload":{"text":"在，我在。"},"complexity_score":2}',
        },
    ]
...
def test_seed_routing_history_recovers_ignore_decision_from_session_meta():
    ...
    assert session.routing_history == [
        {"role": "user", "content": "收到"},
        {"role": "assistant", "content": '{"action":"ignore","payload":{},"complexity_score":1}'},
        {"role": "user", "content": "继续说一下刚才那个问题"},
    ]
...
async def test_run_orchestrator_agent_reuses_local_run_conversation_path(monkeypatch):
    ...
    runner.config = GatewayConfig(
        gateway_orchestrator=GatewayOrchestratorConfig(
            ...
            orchestrator_api_key="minimax-test-key",
    ...
    assert captured["run"]["turn_user_context"] is None
    assert captured["init"]["ephemeral_system_prompt"] is None
    assert captured["init"]["api_mode"] == "anthropic_messages"
    assert captured["init"]["provider"] == "minimax"
    assert captured["init"]["skip_context_files"] is True
    assert captured["init"]["skip_memory"] is True
    assert captured["init"]["runtime_context"].agent_context == "orchestrator"
    assert captured["agent"]._cached_system_prompt == _support(runner).build_system_prompt()
    assert captured["agent"]._build_system_prompt() == _support(runner).build_system_prompt()
...
@pytest.mark.asyncio
async def test_run_orchestrator_agent_uses_configured_runtime_override_and_minimal_reasoning(monkeypatch):
    captured = {}
    ...
    assert captured["init"]["model"] == "minimax-2.7-highspeed"
    assert captured["init"]["provider"] == "minimax"
    assert captured["init"]["base_url"] == "https://api.minimax.io/anthropic"
    assert captured["init"]["api_key"] == "minimax-test-key"
    assert captured["init"]["api_mode"] == "anthropic_messages"
    assert captured["init"]["reasoning_config"] == {"enabled": True, "effort": "minimal"}
...
async def test_run_orchestrator_agent_streams_reasoning_to_observability(monkeypatch):
    ...
    assert reasoning[-1]["content"] == "step 1\nstep 2"
...
@pytest.mark.asyncio
async def test_run_orchestrator_turn_emits_gateway_memory_prefetch_event(monkeypatch):
    import gateway.run as gateway_run
    ...
    runner = _make_runner()
    support = _support(runner)
    original_snapshot_builder = support.build_memory_context_snapshot
    ...
    def _fake_snapshot(**kwargs):
        ...
        return {
            ...
            "provider_error": "",
            "memory_prefetch_by_provider": "## fake-memory\nKnown user preference",
            "memory_prefetch_content": "Known user preference",
            "memory_prefetch_fenced": "<memory-context>\nKnown user preference\n</memory-context>",
            "memory_prefetch_params_by_provider": "## fake-memory\n{\n  \"mode\": \"hybrid\"\n}",
        }

    support.build_memory_context_snapshot = _fake_snapshot
    _runtime(runner).run_orchestrator_agent = AsyncMock(
        return_value={
            "final_response": (
                '{"complexity_score": 1, "respond_now": true, "immediate_reply": "quick answer", '
    ...
    try:
        ...
        support.build_memory_context_snapshot = original_snapshot_builder
    ...
    assert memory_events[0]["source"] == "gateway_prefetch"
    assert memory_events[0]["agent_scope"] == "main_orchestrator"
    assert memory_events[0]["configured_provider"] == "fake-memory"
    assert memory_events[0]["provider_count"] == 1
    assert memory_events[0]["memory_prefetch_content"] == "Known user preference"
    assert memory_events[0]["memory_prefetch_fenced"].startswith("<memory-context>")
    assert memory_events[0]["gateway_memory_context"].startswith("<memory-context>")
    assert memory_events[0]["injection_sources"] == ["memory_provider_prefetch"]
    assert event.metadata[PREFETCHED_MEMORY_CONTEXT_KEY].startswith("<memory-context>")


@pytest.mark.asyncio
async def test_orchestrator_user_correction_is_forced_to_spawn_full_agent():
    runner = _make_runner()
    _runtime(runner).run_orchestrator_agent = AsyncMock(
        return_value={
            "final_response": (
    ...
    assert "收到，我记住了。" not in transcript_replies
...
def test_record_orchestrator_routing_turn_persists_canonical_decision_json():
    ...
    assert session.routing_history[-1]["content"] == (
        '{"action":"spawn","payload":{"goal":"analyze the repo deeply"},"complexity_score":82}'
    )
...
async def test_orchestrator_turn_passes_formatted_routing_history_and_keeps_transcript_natural():
    ...
    assert captured["turn_system_context"] is None
    assert "[Platform note: NapCat platform conversation rules.]" not in captured["turn_user_context"]
    assert "[Context note: Current gateway turn context." in captured["turn_user_context"]
    ...
    assert session.routing_history[-1]["role"] == "assistant"
    assert session.routing_history[-1]["content"] == (
        '{"action":"reply","payload":{"text":"在，我在。"},"complexity_score":1}'
    )
...
@pytest.mark.asyncio
async def test_orchestrator_post_parse_forces_handoff_for_external_info_query():
    runner = _make_runner()
    _runtime(runner).run_orchestrator_agent = AsyncMock(
        return_value={
            "final_response": (
    ...
    assert session.last_decision.respond_now is False
    assert session.last_decision.complexity_score >= 18
    transcript_replies = [call.args[1]["content"] for call in runner.session_store.append_to_transcript.call_args_list]
    assert "晴天" not in transcript_replies
...
@pytest.mark.asyncio
async def test_orchestrator_active_child_status_checkin_never_spawns_new_child():
    runner = _make_runner()
    _runtime(runner).run_orchestrator_agent = AsyncMock(
        return_value={
            "final_response": (
                '{"action":"spawn","payload":{"goal":"你在吗"}}'
            )
        }
    )
    runner._run_agent = AsyncMock()
    ...
    assert "Still working on: query latest earthquake." in response
    assert "Current tool: terminal." in response
    assert runner._run_agent.await_count == 0
    transcript_roles = [call.args[1]["role"] for call in runner.session_store.append_to_transcript.call_args_list]
    assert transcript_roles == ["user"]
...
@pytest.mark.asyncio
async def test_orchestrator_group_message_spawn_is_not_overridden_by_group_gate():
    runner = _make_runner()
    _runtime(runner).run_orchestrator_agent = AsyncMock(
        return_value={
            "final_response": (
...
async def test_orchestrator_child_syncs_visible_assistant_messages_to_transcript_and_routing_history():
    ...
    assert session.routing_history[-2] == {
        "role": "assistant",
    ...
    assert session.routing_history[-1] == {
        "role": "assistant",
        "content": _support(runner).canonicalize_reply_history_json(
            "final child result",
            complexity_score=88,
