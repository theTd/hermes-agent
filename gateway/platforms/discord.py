...
class ChannelFlags:
    """Unified channel-type classification for Discord adapters.

    Replaces the separate *_is_dm_channel_like* / *_is_thread_channel_like*
    helpers so that upstream additions (e.g. guild checks) can be added as
    new fields without introducing new top-level functions that overlap with
    napcat helpers.
    """
...
def _get_channel_type_flags(channel: Any) -> ChannelFlags:
    """Return a ``ChannelFlags`` instance describing *channel*.

    This is the single source of truth for channel-type detection; the
    legacy ``_is_dm_channel_like`` / ``_is_thread_channel_like`` wrappers
    delegate here for backwards compatibility.
    """
    if channel is None:
        return ChannelFlags()

    is_dm = _channel_matches_discord_type(channel, _discord_dm_channel_type(), "dmchannel")
    is_thread = _channel_matches_discord_type(channel, _discord_thread_type(), "thread")

    # Guild detection — covers TextChannel, VoiceChannel, ForumChannel, etc.
    try:
    ...
    # Category detection
    try:
        is_category = getattr(channel, "type", None) is not None and str(channel.type) == "category"
    except Exception:
        is_category = False
...
def _is_dm_channel_like(channel: Any) -> bool:
    """Legacy wrapper — prefer ``_get_channel_type_flags(channel).is_dm``."""
    return _get_channel_type_flags(channel).is_dm


def _is_thread_channel_like(channel: Any) -> bool:
    """Legacy wrapper — prefer ``_get_channel_type_flags(channel).is_thread``."""
    return _get_channel_type_flags(channel).is_thread
