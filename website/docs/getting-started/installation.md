...
If you want to clone the repo and install from source for contributing, running a specific branch, or keeping full control over the environment, follow these steps. For the development workflow itself, also see the [Development Setup](../developer-guide/contributing.md#development-setup) section in the Contributing guide.
...
```bash
git clone --branch napcat --recurse-submodules https://github.com/theTd/hermes-agent.git
cd hermes-agent
```
