...
class NapCatAdapter(BasePlatformAdapter):
    ...
    def allow_gateway_command(
        self,
        source: SessionSource,
        command: Optional[str],
    ) -> tuple[bool, Optional[str]]:
        if source.platform != Platform.NAPCAT or not command:
            return True, None
        if self.is_super_admin(source.user_id):
            return True, None
    ...
    async def handle_passive_event(self, event: MessageEvent, session_store: SessionStore) -> bool:
        ...
        shared_session = is_multi_user_session(
            source,
    ...
    def _resolve_observability_trace_ctx(
        ...
        *,
        ...
        fallback = getattr(self, "_current_trace_ctx", None)
        return fallback if fallback is not None and hasattr(fallback, "trace_id") else None
            ...
            try:
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    if payload.get("echo") in self._pending_requests:
                        ...
                        continue
                    await self._dispatch_incoming_payload(payload)
    ...
    async def _dispatch_incoming_payload(self, payload: dict[str, Any]) -> None:
    ...
    async def _call_action(
        ...
        *,
        ...
        if response.get("status") != "ok" or int(response.get("retcode", 0) or 0) != 0:
            ...
            _ws_action_duration_ms = (time.time() - _ws_action_start) * 1000
            emit_napcat_event(
    ...
    async def _resolve_reply_context(
        ...
        if self._is_internal_control_message_id(message_id):
            emit_napcat_event(
                ...
                trace_ctx=getattr(self, "_current_trace_ctx", None),
            )
        ...
        media_urls, media_types, _cleaned_segments, _detected_type = await self._collect_inbound_media(reply_segments)
    ...
    async def _handle_incoming_event(self, raw_event: dict[str, Any]) -> None:
        ...
        self._current_trace_ctx = inbound_trace_ctx
        ...
        emit_napcat_event(
            EventType.MESSAGE_RECEIVED,
            {
                ...
                "user_id": str(user_id or ""),
                "message_id": str(message_id or ""),
                "text_preview": (_raw_text_preview or "")[:200],
                "segment_count": len(segments),
            },
            trace_ctx=inbound_trace_ctx,
        )
        ...
        media_urls, media_types, text_segments, detected_type = await self._collect_inbound_media(segments)
    ...
    async def _handle_notice_event(self, raw_event: dict[str, Any]) -> None:
    ...
    def _session_has_explicit_model_override(self, source: SessionSource) -> bool:
        ...
        from gateway.session import build_session_key, resolve_session_isolation
    ...
    async def _evaluate_group_trigger(
        ...
        *,
        ...
        group_policy = self._group_policy(group_id)
        user_policy = self._user_policy(user_id)
        _trigger_trace_ctx = getattr(self, "_current_trace_ctx", None)
        ...
        if require_at and not mentioned and not reply_to_bot:
            emit_napcat_event(
                EventType.GROUP_TRIGGER_REJECTED,
                {
    ...
    async def _emit_group_context_only_event(
        ...
        *,
        ...
        event.napcat_trace_ctx = getattr(self, "_current_trace_ctx", None)
        event.gateway_trace_ctx = event.napcat_trace_ctx
        event.metadata = {
            "chat_type": "group",
    ...
    async def _handle_recall_notice(
        ...
        *,
        ...
        self._current_trace_ctx = trace_ctx
    ...
    def _platform_routing_prompt(self, source: SessionSource) -> str:
        ...
        if source.chat_type == "group":
            return (
                ...
                "makes it clear the user is addressing Hermes. If the turn is clearly directed at Hermes, route "
                "the downstream reply toward a compact, natural QQ group style instead of meta narration, "
                "checklists, or system-boundary talk. Do not route host-runtime probe requests into deeper work; "
