...
class NapcatGatewayMixin:
    ...
    # -- Stop-command orchestrator consolidation ------------------------

    async def _cancel_orchestrator_for_stop(self, session_key: str) -> str | None:
        """Cancel orchestrator children for a /stop command.

        Returns a status message string if orchestrator children were
        cancelled, or None if the caller should proceed with the normal
        stop flow (running agent exists, or no orchestrator session).
        """
        extension = self._get_gateway_extension()
        agent = self._running_agents.get(session_key)
        if agent:
            # Running agent takes priority — cancel its orchestrator
            # children but let the caller handle the agent stop.
            await extension.cancel_orchestrator_children(
                session_key, reason="stop requested",
            )
            return None
    ...
    # -- Shutdown / session-boundary orchestrator cleanup ----------------

    async def _shutdown_all_orchestrator_sessions(self, reason: str = "Gateway shutting down") -> None:
        """Cancel children in every orchestrator session during gateway shutdown/restart.

        Consolidates the for-loop over orchestrator_session_keys() that was
        embedded in stop() to reduce merge-conflict surface with upstream.
        """
        extension = self._get_gateway_extension()
        for session_key in extension.orchestrator_session_keys():
            await extension.cancel_orchestrator_children(
                session_key, reason=reason, clear_session=True,
            )

    async def _cancel_orchestrator_at_boundary(self, session_key: str, reason: str) -> None:
        """Cancel orchestrator children + evict cache at a session boundary.

        Used when the session is being reset (auto-reset, /new, compression
        exhaustion) — the orchestrator subtree is no longer valid.
        """
        self._evict_cached_orchestrator_agents(session_key)
