...
def _looks_like_image_url(url: str) -> bool:
    ...
    return any(
        lowered.endswith(ext) or ext in lowered
        for ext in (
            ".png",
            ".jpg",
            ".jpeg",
...
class BasePlatformAdapter(ABC):
    ...
    def extract_images(content: str) -> Tuple[List[Tuple[str, str]], str]:
        ...
        for match in re.finditer(html_pattern, content):
            url = match.group(1)
            if url not in seen_urls and _looks_like_image_url(url):
                images.append((url, ""))
                seen_urls.add(url)
        ...
        for match in re.finditer(bare_url_pattern, content):
            ...
            if url in seen_urls or not _looks_like_image_url(url):
                continue
            images.append((url, ""))
            seen_urls.add(url)
    ...
    def _augment_trace_metadata(
        ...
        trace_attr = str(getattr(cls, "TRACE_CONTEXT_ATTR", "") or "").strip()
        metadata_key = str(getattr(cls, "TRACE_METADATA_KEY", "") or "").strip()
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        async def _process_one_event(active_event: MessageEvent) -> Optional[MessageEvent]:
            ...
            typing_task = asyncio.create_task(
                self._keep_typing(active_event.source.chat_id, metadata=_thread_metadata)
            )
