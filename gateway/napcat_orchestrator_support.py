    ...
    def resolve_child_toolsets(
        ...
        if not selected and source is not None:
            ...
            if platform and platform != "local":
                # Use the full platform toolset list (including dynamically
                # discovered MCP servers) instead of only the composite
                # "hermes-{platform}" name.  Otherwise child agents miss
                # every MCP tool because the composite name does not expand
                # to the per-server toolset aliases registered by MCP discovery.
                raw_config = getattr(self.host, "gateway_config", None)
                user_config = raw_config() if callable(raw_config) else {}
                if isinstance(user_config, dict):
                    from hermes_cli.tools_config import _get_platform_tools
                    selected = list(_get_platform_tools(user_config, platform))
                else:
