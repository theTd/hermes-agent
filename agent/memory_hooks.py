...
def is_explicit_memory_request(text: str) -> bool:
    """Return True when the user explicitly asks for long-term memory.

    Intentionally narrow: catches imperative "remember this" requests without
    firing on casual recall questions like "do you remember...".
    """
    ...
    lower = raw.lower()
    if re.search(r"\b(do|did|can|could|would|will)\s+you\s+remember\b", lower):
        return False
    if re.search(r"\bremember\s+when\b", lower):
        return False

    english_patterns = (
        r"^remember\b",
        r"\bplease\s+remember\b",
        r"\bremember\s+(?:this|that|it|me|my|the|for later)\b",
        r"\bdon't\s+forget\b",
        r"\bnote\s+(?:this|that|it|down)\b",
        r"\bsave\s+(?:this|that|it).{0,24}\b(?:to|in)\s+memory\b",
    )
    chinese_patterns = (
        "记住",
        "记一下",
    ...
    if any(re.search(pattern, lower) for pattern in english_patterns):
        return True
    return any(token in raw for token in chinese_patterns)
