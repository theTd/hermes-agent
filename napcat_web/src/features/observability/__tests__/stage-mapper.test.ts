...
describe("mapTraceToLiveGroup", () => {
  ...
  it("normalizes cumulative reasoning content into incremental steps", () => {
    const group = mapTraceToLiveGroup(
    ...
    expect(block?.thinkingStepCount).toBe(2);
    expect(block?.thinkingSteps).toEqual([
      expect.objectContaining({
        sequence: 1,
        source: "suffix",
        text: "first pass reasoning",
      }),
      expect.objectContaining({
        sequence: 2,
        source: "suffix",
  ...
  it("deduplicates matching delta and content updates while preserving resets", () => {
    const group = mapTraceToLiveGroup(
