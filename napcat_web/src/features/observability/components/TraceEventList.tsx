...
function compactStageSummary(summary: string, maxChars: number = 220): string {
    ...
    <div className="space-y-3">
      {group.stages.map((stage) => (
        <div key={stage.key} className="border-l border-border/40 pl-3">
          <div className="text-[10px] font-compressed uppercase tracking-widest opacity-50">{stage.title}</div>
          <div className="text-sm opacity-85">{compactStageSummary(stage.summary)}</div>
        </div>
      ))}
