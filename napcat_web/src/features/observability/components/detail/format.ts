...
export function formatPayloadPreview(value: unknown, maxChars: number = 1600): string {
  try {
    const json = JSON.stringify(
      ...
      (_key, innerValue) => {
        if (typeof innerValue === "string") {
          if (innerValue.length > 240) {
            return `${innerValue.slice(0, 240)}… [truncated ${innerValue.length - 240} chars]`;
          }
          return innerValue;
        }
        if (Array.isArray(innerValue) && innerValue.length > 20) {
          return [
            ...innerValue.slice(0, 20),
            `… [truncated ${innerValue.length - 20} items]`,
          ];
        }
        return innerValue;
      },
    ...
    return json.length > maxChars
      ? `${json.slice(0, maxChars)}\n… [payload preview truncated ${json.length - maxChars} chars]`
      : json;
  } catch {
...
export function getExpandableTextPanelVisibility(options: {
  ...
  if (normalizedTitle === "thinking" || normalizedTitle === "response") {
    return "preview";
  }
  if (normalizedTitle === "debug payload" || normalizedTitle === "request body") {
    return "hidden";
  }
