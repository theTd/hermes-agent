...
# ---------------------------------------------------------------------------
# Auto-resolve rules for napcat-specific conflict regions.
#
# When a conflict is detected in a file listed here, the script can
# automatically resolve it by taking the upstream version and re-applying
# small napcat-specific patches.  This avoids manual resolution for
# regions where our changes are minimal and well-understood.
# ---------------------------------------------------------------------------

NAPCAT_AUTO_RESOLVE_RULES: dict[str, dict] = {
    "gateway/run.py": {
        "strategy": "take_upstream_then_patch",
        "description": (
            "stop() orchestrator cleanup is now a single mixin call. "
            "Upstream changes to stop() are accepted; the mixin call is "
            "re-inserted after the drain loop."
        ),
        "post_patches": [
            {
                # Insert the consolidated orchestrator shutdown call right
                # after the drain-timeout interrupt loop ends.
                "after_pattern": r"^\s{20}await asyncio\.sleep\(0\.1\)",
                "insert_block": (
                    "            await self._shutdown_all_orchestrator_sessions(\n"
...
def get_conflicting_files(repo_root: Path) -> list[str]:
    """Return the list of files with unmerged paths."""
    result = run_git(
        repo_root,
        "diff",
        "--name-only",
...
def apply_post_patches(repo_root: Path, filepath: str, patches: list[dict]) -> bool:
    """Apply post-resolution patches to *filepath*.

    Each patch dict must have:
      - ``after_pattern``: a regex matching the line *after* which the
        insert block should be placed.
      - ``insert_block``: the text to insert.
    ...
    try:
        ...
        print(f"  WARNING: {filepath} not found for patching", file=sys.stderr)
        return False

    for patch in patches:
        pattern = patch["after_pattern"]
        insert = patch["insert_block"]
        match = re.search(pattern, text, re.MULTILINE)
        if not match:
            print(
                f"  WARNING: patch pattern not found in {filepath}: {pattern}",
                file=sys.stderr,
            )
            return False
...
def try_auto_resolve(repo_root: Path) -> bool:
    """Attempt auto-resolve for conflicting files with registered rules.

    Returns True if *all* conflicts were resolved automatically.
    """
    conflicting = get_conflicting_files(repo_root)
    if not conflicting:
        return True

    resolved_any = False
    for filepath in conflicting:
        if filepath not in NAPCAT_AUTO_RESOLVE_RULES:
            continue

        rule = NAPCAT_AUTO_RESOLVE_RULES[filepath]
        strategy = rule.get("strategy")

        if strategy == "take_upstream_then_patch":
            print(f"  Auto-resolving {filepath}: taking upstream + re-applying napcat patches")
            # Take upstream's version
            run_git(repo_root, "checkout", "--theirs", filepath)
            # Apply napcat-specific post-patches
            patches = rule.get("post_patches", [])
            if patches:
                if not apply_post_patches(repo_root, filepath, patches):
                    print(f"  WARNING: post-patch failed for {filepath}, skipping", file=sys.stderr)
                    continue
            # Mark as resolved
            run_git(repo_root, "add", filepath)
            resolved_any = True
        else:
            print(f"  WARNING: unknown strategy '{strategy}' for {filepath}", file=sys.stderr)

    if resolved_any:
        # Check if all conflicts are now resolved
        remaining = get_conflicting_files(repo_root)
        if not remaining:
            return True
    ...
    for index, upstream_commit in enumerate(upstream_commits, start=1):
        ...
        if result.returncode != 0:
            if has_unmerged_paths(repo_root) or has_rebase_head(repo_root):
                # Try auto-resolve for files with registered rules
                if try_auto_resolve(repo_root):
                    # Auto-resolved — continue the rebase
                    cont = run_git(
                        repo_root,
                        "-c", "core.editor=true",
                        "rebase", "--continue",
                    ...
                    if cont.returncode == 0:
                        print(f"  Auto-resolved conflict for: {subject}")
