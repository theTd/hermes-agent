...
@pytest.mark.asyncio
async def test_prepare_inbound_message_text_enriches_referenced_napcat_group_image(monkeypatch, tmp_path):
    from gateway.run import GatewayRunner
    ...
    async def _fake_vision(*, image_url, user_prompt):
        ...
        return '{"success": true, "analysis": "A screenshot of a deployment error."}'
    ...
    assert "A screenshot of a deployment error." in result
    assert "What does this pic mean" in result
