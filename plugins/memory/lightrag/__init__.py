...
class LightRAGMemoryProvider(MemoryProvider):
    ...
    def _headers(self, workspace: str | None = None) -> dict[str, str]:
        ...
        # Passed workspace takes precedence over self._workspace so per-scope
        # writes/queries can target the correct workspace instead of always
        # falling back to the default.
        if workspace is not None:
            workspace_name = str(workspace).strip()
            if workspace_name:
                headers["LIGHTRAG-WORKSPACE"] = workspace_name
        elif self._workspace:
            headers["LIGHTRAG-WORKSPACE"] = self._workspace
        return headers

    def _expand_query_top_k(self, top_k: int | None, scope_count: int) -> int | None:
        if top_k is None:
            return None
        # When querying per-workspace we no longer need to over-fetch to
        # compensate for client-side filtering across scopes.
        multiplier = max(1, scope_count) * 4
        return min(80, max(top_k, top_k * multiplier))

    def _query_data_for_workspaces(
        self,
        ...
        *,
        ...
        """Query LightRAG per-workspace and merge results.

        Each workspace gets its own scoped request so server-side isolation
        prevents cross-group leakage. Falls back to a single request when
        there is only one workspace.
        """
        if len(workspaces) <= 1:
            workspace = workspaces[0] if workspaces else None
            result = self._request_json("POST", "/query/data", payload, workspace=workspace)
        ...
        for workspace in workspaces:
            try:
                ...
                logger.debug("LightRAG per-workspace query failed for %s", workspace, exc_info=True)
                continue

        # Deduplicate merged items by their JSON representation.
        for key in merged:
            seen: set[str] = set()
            deduped: list[Any] = []
        ...
        if log:
            logger.debug(
                "LightRAG per-workspace merged: workspaces=%s lane=%s entities=%s relationships=%s chunks=%s references=%s",
                workspaces,
                lane,
                len(merged.get("entities") or []),
                len(merged.get("relationships") or []),
                len(merged.get("chunks") or []),
                len(merged.get("references") or []),
            )
        return merged
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        self._last_prefetch_debug.update({
            "strategy": "query_data_per_workspace",
            "render_scopes": list(render_scopes),
            "request_payload": request_payload,
            "api_called": True,
        })
        data = self._query_data_for_workspaces(
            request_payload,
            render_scopes,
            lane=query_plan.lane,
        )
    ...
    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        ...
        if tool_name == "lightrag_search":
            ...
            raw_data = self._query_data_for_workspaces(
                payload,
                workspaces,
                lane=query_plan.lane,
            )
