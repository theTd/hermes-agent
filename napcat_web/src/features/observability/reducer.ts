...
function pruneEntities(
  ...
  if (selectedTraceId && entities.tracesById[selectedTraceId]) {
...
export function observabilityReducer(
  ...
  switch (action.type) {
    case "bootstrap_from_snapshot": {
      ...
      const initialMergedTraces = action.payload.traces
        ? {
            ...baseTraces,
            ...normalizeTraces(action.payload.traces),
          }
        : baseTraces;
      const traceEvents = normalizeEvents(action.payload.traces);
      const payloadEvents = normalizeEventList(action.payload.events);
      ...
      const mergedEvents = mergeEventMaps(
        baseEvents,
        mergeEventMaps(traceEvents, payloadEvents),
      );
      ...
      const mergedAlerts = action.payload.alerts
        ? {
            ...baseAlerts,
            ...normalizeAlerts(action.payload.alerts),
          }
        : baseAlerts;
      const mergedSessions = action.payload.sessions
        ? (
            replaceEntities
              ? normalizeSessions(action.payload.sessions, action.payload.traces)
              : {
      ...
      const selectedTraceId = state.connection.followLatest
        ...
        : state.ui.selectedTraceId && mergedTraces[state.ui.selectedTraceId]
          ...
          : state.ui.selectedTraceId ?? action.payload.traces?.[0]?.trace_id ?? null;
