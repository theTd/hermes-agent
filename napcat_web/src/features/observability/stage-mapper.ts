...
export function fallbackStageForEvent(event: NapcatEvent): TraceStageKey {
  ...
  if (eventType === "agent.memory.used" || eventType === "agent.skill.used" || eventType === "policy.applied") return "memory_skill_routing";
  if (
...
function buildStreamChunks(events: NapcatEvent[]): StreamChunk[] {
  ...
  for (const event of events) {
    ...
    chunks.push({
      kind: event.event_type === "agent.reasoning.delta" ? "thinking" : "response",
      content: String(event.payload?.content || event.payload?.delta || ""),
      ts: event.ts,
      sequence: Number(event.payload?.sequence ?? 0),
      stage: fallbackStageForEvent(event),
      source: String(event.payload?.source || ""),
      agentScope: String(event.payload?.agent_scope || ""),
      childId: String(event.payload?.child_id || ""),
    });
  }
...
function summarizeStage(key: TraceStageKey, events: NapcatEvent[], toolCalls: ToolCallItem[], streamChunks: StreamChunk[]): string {
  ...
  switch (key) {
    ...
    case "trigger": {
      ...
      if (rejected) {
        return `Rejected: ${String(rejected.payload?.final_reason || rejected.payload?.rejection_layer || "blocked").slice(0, 120)}`;
      }
      const evaluated = latestOf(events, "group.trigger.evaluated");
      if (evaluated) {
        return `Accepted: ${String(evaluated.payload?.trigger_reason || "triggered").slice(0, 120)}`;
      }
      const contextOnly = latestOf(events, "group.context_only_emitted");
      if (contextOnly) {
        return `Context only: ${String(contextOnly.payload?.trigger_reason || "no reply").slice(0, 120)}`;
      }
    ...
    case "model": {
      ...
      return String(lastRequest?.payload?.model || "Model activity").slice(0, 160);
    }
    case "tools":
...
function pushThinkingStep(
  steps: ThinkingStep[],
  candidate: {
    sequence: number;
    ts: number;
    text: string;
    source: ThinkingStep["source"];
  },
): void {
  ...
  const lastComparable = normalizeComparableText(last.text);
  if (lastComparable === nextComparable) {
    return;
  }
  if (lastComparable && nextComparable.startsWith(lastComparable)) {
    last.sequence = candidate.sequence;
    last.ts = candidate.ts;
    last.text = nextText;
    last.source = candidate.source;
    return;
  }
  if (lastComparable && lastComparable.startsWith(nextComparable)) {
    return;
  }
...
function applyThinkingDelta(block: MutableRequestBlock, event: NapcatEvent): void {
  ...
  const addCandidate = (text: string, source: ThinkingStep["source"]) => {
  ...
  if (delta.trim()) {
    addCandidate(delta, "delta");
  }

  let nextFullText = currentFullText;
  if (content) {
    if (!currentFullText || content.startsWith(currentFullText)) {
      const suffix = currentFullText ? content.slice(currentFullText.length) : content;
      if (suffix.trim()) {
        addCandidate(suffix, "suffix");
      }
      ...
      nextFullText = content;
      candidates.length = 0;
      seenComparables.clear();
      addCandidate(content, "reset");
    }
  } else if (delta.trim()) {
  ...
  for (const candidate of candidates) {
    pushThinkingStep(block.thinkingSteps, candidate);
  }

  block.thinkingFullText = nextFullText;
  block.thinkingText = nextFullText;
  block.thinkingStepCount = block.thinkingSteps.length;
}
...
function timelineEntryKind(event: NapcatEvent): TimelineEntry["kind"] {
  ...
  if (event.event_type === "agent.skill.used") {
    return "skill";
  }
  ...
  for (const event of events) {
    ...
    if (requestId) {
      ...
      switch (event.event_type) {
        ...
        case "agent.model.completed":
          ...
          block.durationMs = Number(event.payload?.duration_ms) || Math.max(0, (event.ts - block.startedAt) * 1000);
    ...
    if (shouldRenderStandaloneTimelineEntry(event) && !requestLocalOnly) {
      lane.timelineEntries.push({
        ...
        status: event.event_type.endsWith(".failed") || event.event_type === "error.raised" ? "error" : "completed",
        durationMs: Number(event.payload?.duration_ms) || null,
        payloadPreview: trimPayloadPreview(event.payload),
        event,
      });
    }
  }
